function randomNumber(bytes) {
    var byteArray = Array(bytes).fill(0);
...
var getMessageId = (function () {
    ...
    return function () {
        var id = getUnixtime() * multiplier;
        id += id % 4;
        while (lastMessageId >= id) {
            id += 4;
        }
        lastMessageId = id;
        return id;
...
function sendRequest(byteArray) {
    return new Promise((resolve, reject) => {
        ...
        request.open('POST', 'https://venus.web.telegram.org/apiw_test1', true);
...
var selectRsaKeyByFingerPrint = (function () {
    var publisKeysHex = [
        {
...
function saveServerTimeOffset(serverUnixTime) {
